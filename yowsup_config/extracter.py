...
class Extracter():
    ...
    def extractFromDevice(self, serial=None, package: str = 'com.whatsapp', dirpath: str = tempfile.gettempdir()) -> str:
        ...
        config_dirpath = self.extractSharedPreference(device, package, dirpath)
        logger.info('done')
        # extract db file
        logger.info('extract axolotl db file')
        self.extractAxolotlDatabase(device, package, config_dirpath)
        logger.info('done')
        logger.info(f'save to {config_dirpath}')
        logger.info('Finish')
        return config_dirpath

    def extractSharedPreference(self, device: AdbWrapper, package: str, dirpath: str):
        # parse keystore.xml
        ...
        keystore = self.parsePrefs(device, keystore_filepath)
        ...
        if not client_static_keypair:
            ...
            client_static_keypair = self.decryptKeyPair(device, client_static_keypair_encrypted)
        ...
        server_static_public = self.b64padding(server_static_public)
        ...
        prefs = self.parsePrefs(device, prefs_light_filepath)
        phone = prefs.get('registration_jid')
        cc = prefs.get('cc')
        carrier = self.choiceCarrier(cc)
        mcc = carrier['mcc']
        mnc = carrier['mnc']
        sim_mcc = carrier['mcc']
        sim_mnc = carrier['mnc']
        edge_routing_info = self.b64padding(prefs.get('routing_info'))
    ...
    def extractAxolotlDatabase(self, device: AdbWrapper, package: str, dirpath: str):
    ...
    def decryptKeyPair(self, device: AdbWrapper, keypair_enc: str) -> str:
    ...
    def choiceCarrier(self, country_code: str):
        dirpath = os.path.dirname(os.path.abspath(__file__))
    ...
    def parsePrefs(self, device: AdbWrapper, device_prefs_file: str) -> Dict[str, str]:
